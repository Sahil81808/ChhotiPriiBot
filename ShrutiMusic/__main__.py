...
async def init():
    ...
    for all_module in ALL_MODULES:
        importlib.import_module("ChhotiPriiBot.plugins" + all_module)
    LOGGER("ChhotiPriiBot.plugins").info("Successfully Imported Modules...")
    await userbot.start()
    await Aviax.start()
    try:
        await Aviax.stream_call("https://te.legra.ph/file/29f784eb49d230ab62e9e.mp4")
    except NoActiveGroupCall:
        LOGGER("ChhotiPriiBot").error(
    ...
    await app.stop()
    await userbot.stop()
    LOGGER("ChhotiPriiBot").info("Stopping Shruti Music Bot...")
